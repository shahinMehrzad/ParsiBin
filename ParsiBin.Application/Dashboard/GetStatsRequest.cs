using ParsiBin.Application.Common.Persistence;
using ParsiBin.Application.Identity.Roles;
using ParsiBin.Application.Identity.Users;

namespace ParsiBin.Application.Dashboard
{
    ...
    public class GetStatsRequestHandler : IRequestHandler<GetStatsRequest, StatsDto>
    {
        ...
        public async Task<StatsDto> Handle(GetStatsRequest request, CancellationToken cancellationToken)
        {
            ...
            {
                //ProductCount = await _productRepo.Count(cancellationToken),

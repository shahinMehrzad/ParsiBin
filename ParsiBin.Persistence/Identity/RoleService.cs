...
using ParsiBin.Application.Common.Exceptions;
using ParsiBin.Application.Common.Interfaces;
using ParsiBin.Application.Identity.Roles;
using ParsiBin.Domain.Identity;
...
namespace ParsiBin.Persistence.Identity
{
    internal class RoleService : IRoleService
    {
        ...
        private readonly ParsibinContext _db;
        ...
        public async Task<RoleDto> GetByIdWithPermissionsAsync(string roleId, CancellationToken cancellationToken)
        {
            ...
            role.Permissions = await _db.RoleClaims
                .Where(c => c.RoleId == roleId && c.ClaimType == ParsiBinClaims.Permission)
        ...
        public async Task<string> UpdatePermissionsAsync(UpdateRolePermissionsRequest request, CancellationToken cancellationToken)
        {
            ...
            foreach (string permission in request.Permissions.Where(c => !currentClaims.Any(p => p.Value == c)))
            {
                if (!string.IsNullOrEmpty(permission))
                {
                    ...
                    {
                        RoleId = role.Id,
                        ClaimType = ParsiBinClaims.Permission,
